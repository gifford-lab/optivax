...
if __name__ == "__main__":
    ...
    pred_file1='_'.join(['mhc1_haplotype', 'mean', 'pred_affinity', 'pivot.pkl.gz'])
    pred_file2='_'.join(['mhc2_haplotype', 'netmhcii-4.0', 'pred_affinity', 'pivot.pkl.gz'])
    ...
    def process_predict(pred_file,args,objectives,selfp,basement=None,basefile=None,mtype='mhc1'):
    ...
    if args.basement or args.basefile:
        for mhc in ['mhc1','mhc2']:
            prediction[mhc],basecount[mhc],base_hist[mhc],pre_base[mhc]=process_predict(pred_files[mhc],args,objectives,selfp,\
        ...
        for mhc in ['mhc1','mhc2']:
            prediction[mhc],basecount[mhc],base_hist[mhc],pre_base[mhc]=process_predict(pred_files[mhc],args,objectives,selfp,mtype=mhc)
        basedir=None
        base_hist=None

    if not exists(join(args.outdir,'plots')):
        makedirs(join(args.outdir,'plots'))
    candidate_list=pd.read_pickle('preprocess_len25_all.pkl')
